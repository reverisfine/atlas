...
FREQ.forEach(([k,v])=>{
  const r=el('div','frow',`<div class="fl">${k}</div><div class="ft"><div class="fb">${v}회</div></div>`);
  r._w=(v/FREQ[0][1]*100)+'%';$('freq').appendChild(r);
});
$('cnote').innerHTML=CNOTE;
function animateCharts(){
  document.querySelectorAll('.barcol').forEach(c=>c.querySelector('.gbar').style.height=c._h);
  document.querySelectorAll('.frow').forEach(r=>r.querySelector('.fb').style.width=r._w);
}
